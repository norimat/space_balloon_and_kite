        ...
        try:
            ...
            if os.path.isfile( self.__parameterDic["input_dir"] + "/" + "movie.h264" ):
                ...
                if (
                    ...
                    iai.getMergeDataFrame() # get dataFrame
                    # threadList.append(
                    #     threading.Thread(
                    #         target= mai.doMovieAnalyzerImpl(
...
class I2CAnalyzerImpl:
    ...
    def __generate_alititude_csv( self ):
        ...
        self.__mergeDataFrame = self.__mergeDataFrame.reset_index()
        self.__mergeDataFrame['altitude'] = self.__mergeDataFrame.apply(
            lambda row :  self.__calculate_altitude(
                row['temperature'] , row['humidity'] , row['pressure'] ) , axis=1
        )
        self.__mergeDataFrame['current_time'] = self.__mergeDataFrame['unix_epoch_time'].apply(
...
class MovieAnalyzerImpl:
    ...
    def __add_sensor_frame( self , dataFrame , framerate ):
        ...
        for imgFile in imgFiles:
            ...
            if self.__parameterDic["bme280_csv"] is not None:
                if self.__parameterDic["altitude_en"]:
            ...
            if self.__parameterDic["mpu6050_csv"] is not None:
            ...
            if self.__parameterDic["icm20948_csv"] is not None:
            ...
            if self.__parameterDic["gps_csv"] is not None:
    ...
    #########################################################################
    def doMovieAnalyzerImpl( self , frame_sync_en , movieFileName ):
        if frame_sync_en :
