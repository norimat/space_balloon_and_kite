...
## 2. IMX219またはIMX708を用いた動画取得について
### 2-1. IMX219およびIMX708について
### 2-2. センサー取得モード時の動画ファイルおよび出力csvファイル
### 2-3. センサーデータ解析モード時の動画ファイルおよび出力csvファイル

## 3. BME280またはBMP280を用いた高度算出について
### 3-1. BME280およびBMP280について
...
##### 3-2-3-2. 混合比(Mixing Ratio)
```math
r=\frac{0.622×e}{P-e}
```

##### 3-2-3-3. 水蒸気圧(Vapor Pressure)
```math
e=e_s×\left( \frac{RH}{100} \right)
```
<!--------------------------------------------------------------------------------->

##### 3-2-3-4. 飽和水蒸気圧(Saturation Vapor Pressure)
...
## 4. MPU6050を用いた加速度および角速度の取得について

## 5. MPU9250を用いた加速度、角速度および地磁気の取得について
現在、MPU9250生産終了のためICM20948に以降予定。

## 6. ICM20948を用いた加速度、角速度および地磁気の取得について
